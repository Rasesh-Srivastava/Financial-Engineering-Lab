...
def compare_volatility(file, opt_type):
    ...
    stocks_df = pd.read_csv('./q2c/nsedata1.csv')
    stocks_df['Date']= pd.to_datetime(stocks_df['Date'])
    stocks_df = stocks_df.interpolate(method='ffill', axis=0)
    ...
    for index, row in options_df.iterrows():
        ...
        if coin_toss <= bound:
            d1 = datetime.strptime(row['Expiry'],  '%m/%d/%Y')
...
files = ['NIFTYoptiondata', 'stockoptiondata_TATAMOTORS', 'stockoptiondata_HDFCBANK']
opt_type = ['Nifty', 'TATAMOTORS.NS', 'HDFCBANK.NS']

for idx in range(len(files)):
    files[idx] = './q2c/' + files[idx] + '.csv'
